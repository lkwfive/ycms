...
class GdWatermark
{
	...
	public function createWatermark($watermark_path, $proportion=0, $right_margin=0, $bottom_margin=0, &$that)
	{
		//配置
		$options = $that->getOptions();
		$proportion = $proportion ? $proportion : $options['proportion'];
		$right_margin = $right_margin ? $right_margin : $options['right_margin'];
		$bottom_margin = $bottom_margin ? $bottom_margin : $options['bottom_margin'];

		//原图
		$oimage=$that->getWorkingImage() ? $that->getWorkingImage() : $that->getOldImage();
		//水印图片
		$wimage=imagecreatefrompng($watermark_path);
	    ...
	    imagecopy($oimage, 
	    	$new_wimage, 
	    	$oimage_w - $new_wimage_w - $right_margin, 
	    	$oimage_h - $new_wimage_h - $bottom_margin, 
	    	0, 
	    	0, 
	    	$new_wimage_w, 
	    	$new_wimage_h);
